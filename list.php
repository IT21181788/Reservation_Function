...
<body>
    ...
    <div class="container">
        ...
        <form method="GET" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            ...
            <input type="text" name="search" id="search" placeholder="Enter guest name to search...">
    ...
    <br><br><br> 
        ...
        <div id="search-results">
            ...
            if (isset($_GET['search'])) {
                ...
                $stmt->execute();
                $filteredReservations = $stmt->fetchAll(PDO::FETCH_ASSOC);
                ...
                if (count($filteredReservations) > 0) {
                    ...
                    foreach ($filteredReservations as $reservation) {
                        ...
                        echo '<td>' . $reservation['check_in_date'] . '</td>';
                        echo '<td>' . $reservation['check_out_date'] . '</td>';
                        echo '<td>' . $reservation['num_guests'] . '</td>';
                        echo '<td>
                                <button class="edit-button" onclick="editReservation(' . $reservation['reservation_ID'] . ')">Edit</button>
                                <button class="delete-button" onclick="deleteReservation(' . $reservation['reservation_ID'] . ')">Delete</button>
                            </td>';
                        echo '</tr>';
                    }
